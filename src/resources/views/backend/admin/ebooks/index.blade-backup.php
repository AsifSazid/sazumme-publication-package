                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <div class="overflow-x-auto">
                            ...
                            {{-- <tbody class="text-center">
                                @forelse ($ebooks as $ebook)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4">{{ $ebook->user->name }}</td>
                                        <td class="px-6 py-4">{{ $ebook->starts_at }}</td>
                                        <td class="px-6 py-4">{{ $ebook->ends_at }}</td>
                                        <td class="px-6 py-4">{{ $ebook->is_active ? 'Active' : 'Inactive' }}
                                        </td>
                                        <td class="px-6 py-4">
                                            <div class="flex justify-center space-x-2">
                                                <a href="{{ route('admin.ebooks.show', $ebook->uuid) }}"
                                                ...
                                                <form
                                                    action="{{ route('admin.ebooks.destroy', $ebook->uuid) }}"
                                ...
                                @empty
                                    <tr>
                                        <td colspan="7" class="py-4 text-center text-gray-500">No ebooks found
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody> --}}
                        ...
                        <div
                            ...
                            <a href="{{ route('admin.ebooks.trash') }}" class="text-red-500 hover:text-red-700 mx-1"
                ...
                const renderTable = (ebooks) => {
                    ...
                    ebooks.forEach((ebook, index) => {
                        ...
                        <tr>
                            ...
                            <td class="px-6 py-4">${ebook.title}</td>
                            <td class="px-6 py-4">${ebook.user?.name ?? 'N/A'}</td>
                            <td class="px-6 py-4">${formatDateTime(ebook.starts_at)}</td>
                            <td class="px-6 py-4">${formatDateTime(ebook.ends_at)}</td>
                            <td class="px-6 py-4">${ebook.is_active ? 'Active' : 'Inactive'}</td>
                            <td class="px-6 py-4">
                                <div class="flex justify-center">
                                    <a href="/ebooks/${ebook.uuid}" class="px-1 text-blue-500 hover:text-blue-700" title="View"><i class="fas fa-eye"></i></a>
                                    <a href="/ebooks/${ebook.uuid}/edit" class="px-1 text-yellow-500 hover:text-yellow-700" title="Edit"><i class="fas fa-edit"></i></a>
                                    <form action="/ebooks/${ebook.uuid}" method="POST" onsubmit="return confirm('Move to trash?')">
