                        ...
                        <form action="{{ route('admin.ebooks.update', $ebook->id) }}" method="POST"
                            ...
                            <div class="mb-4">
                                <label for="announcement_for"
                                    class="block text-sm font-medium text-gray-700">Ebook For</label>
                                <select name="announcement_for" id="announcement_for"
                                    class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
                                    <option value="" disabled>Select Any One</option>
                                    <option value="main_website"
                                        {{ old('announcement_for', $ebook->announcement_for) === 'main_website' ? 'selected' : '' }}>
                                        Main Website</option>
                                    @foreach ($wings as $wing)
                                        <option value="{{ $wing->id }}"
                                            {{ old('announcement_for', $ebook->announcement_for) == $wing->id ? 'selected' : '' }}>
                                            {{ $wing->title }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="mb-4">
                                ...
                                <textarea name="body" id="body" rows="5" required
                                    class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">{{ old('body', $ebook->body) }}</textarea>
                            </div>
                            ...
                            @if ($ebook->image)
                                <div class="mb-6 flex justify-center">
                                    <img src="{{ asset('storage/' . $ebook->image->url) }}" alt="Current Image"
                            ...
                            <div class="mb-4">
                                ...
                                <input type="datetime-local" name="starts_at" id="starts_at"
                                    value="{{ old('starts_at', optional($ebook->starts_at)->format('Y-m-d\TH:i')) }}"
                                    class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
                            </div>
                            ...
                            <div class="mb-4">
                                ...
                                <input type="datetime-local" name="ends_at" id="ends_at"
                                    value="{{ old('ends_at', optional($ebook->ends_at)->format('Y-m-d\TH:i')) }}"
                                    class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
                            </div>
